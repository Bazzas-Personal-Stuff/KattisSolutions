...
/*
    ...
    Value and flag can be accessed with bitwise AND
        ...
        bool flag = world[x][y] & 2 > 1;
*/
vector<vector<char>> world;
bool checkValidPoint(pair<int, int> point, char type) {
    if (point.first >= 0 && point.first < world[0].size() &&
        point.second >= 0 && point.second < world.size()) { // in map bounds
        char element = world[point.first][point.second];
        if (!(element & 2 > 1) &&
            (element & 1) == type) { // not visited and same type
            return true;
        }
    }
    return false;
...
int main() {
    ...
    for (int i = 0; i < yMax; i++) {
        for (int j = 0; j < xMax; j++) {
            char c;
            cin >> c;
            world[j][i] = c - '0'; // convert from ascii to number
        }
    }
    ...
    for (int i = 0; i < queryCount; i++) {
        ...
        string originString;
        cin >> origin.second >> origin.first >> destination.second >>
            destination.first;
